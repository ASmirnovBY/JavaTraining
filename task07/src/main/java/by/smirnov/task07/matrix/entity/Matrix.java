package by.smirnov.task07.matrix.entity;

import by.smirnov.task07.matrix.exceptions.MatrixException;

public class Matrix {
    private int[ ][ ] a;
    public Matrix(int n) throws MatrixException { // конструктор для квадратичной матрицы
        if ((n < 1)) {
            throw new MatrixException("Matrix size must be > 0");
        }
        a = new int[n][n];
    }
    ...
    public void setElement(int i, int j, int value) throws MatrixException {
        if (checkRange(i, j)) { // проверка i и j
            ...
            throw new MatrixException("Incorrect i or j");
        }
